...
resumen %>%
  ...
  kable_classic(full_width = FALSE, html_font = "Times New Roman") %>%
  save_kable("graficos/resumen_por_grupo.html")
...
```{r}
### 3. DIFERENCIA PROMEDIO ---------------------- version Lau ------------------- 
...
tabla_ic %>%
  kbl(
    caption = "Tabla 3. Estimación bootstrap de medias e intervalos de confianza por grupo de PIB (2023)",
    align = "lc",
    booktabs = TRUE,
    digits = 3
  ) %>%
  kable_classic(full_width = FALSE, html_font = "Times New Roman") %>%
  save_kable("graficos/tabla_ic.html")
...
#2.1 Estime la diferencia promedio de emisiones entre los grupos e interprete el sentido y magnitud de la diferencia 
...
save_kable(
  tabla_diferencia, 
  file = "Graficos/tabla_ic.html"
) #falta interpretación de esta diferencia tanto en el sentido como en la magnitud 



# 2.2 Estime un intervalo de confianza del 95 % para esa diferencia usando un procedimiento Bootstrap y una prueba t
...
cat(tabla_boot)
# Guardar la tabla en HTML (siguiendo su solicitud previa)
save_kable(tabla_boot, file = "graficos/tabla_ic.html")
...
save_kable(tabla_final, file = "graficos/tabla_ic_comparada.html")
